...
internal object DefaultCleanupPrompts {
    private const val TranscriptPlaceholder = CleanupPromptFactory.TranscriptPlaceholder
    ...
    fun isLegacyBuiltInDefault(prompt: String): Boolean {
        if (!prompt.contains(TranscriptPlaceholder)) return false
        return legacyPrompts().any { legacyPrompt ->
            prompt == legacyPrompt.trim()
        }
    }

    private fun legacyPrompts(): List<String> = listOf(
        """
            Please turn the following voice transcript into natural text that can be pasted directly into the current text field.
